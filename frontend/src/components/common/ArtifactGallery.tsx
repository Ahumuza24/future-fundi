...
const truncate = (value?: string | null, maxLength: number = 20) => {
...
const renderPreview = (artifact: ArtifactCardData) => {
        ...
        <Card key={artifact.id} className="border-gray-200 shadow-sm">
          <CardContent className="space-y-3 p-4">
            <div className="flex items-start justify-between gap-2">
              <div>
                ...
                <p className="text-[11px] text-gray-500">
                  {artifact.moduleName ?? 'General module'} · {formatDateTime(artifact.submittedAt)}
                </p>
              </div>
              <Badge
            ...
            {artifact.rejectionReason && (
              <p className="text-[11px] text-red-500">Reason: {artifact.rejectionReason}</p>
            )}
