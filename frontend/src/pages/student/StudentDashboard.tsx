              ...
              <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 }}>
                <Card className="border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  <CardContent className="space-y-4 pb-5">
                    {dashboardData.pathways.map((p) => {
                      const Icon = getIconComponent(p.icon);
                      return (
                        <div key={p.id}
                          className="cursor-pointer group"
                          onClick={() => navigate(`/student/pathway/${p.id}`)}
                        >
                          <div className="flex items-center gap-2 mb-1.5">
                            <div className="p-1.5 rounded-lg shrink-0" style={{ backgroundColor: `${p.color}20` }}>
                              <Icon className="h-3 w-3" style={{ color: p.color }} />
                            </div>
                            <div className="flex-1 min-w-0">
                              <p className="text-xs font-bold text-gray-800 truncate group-hover:text-[var(--fundi-orange)] transition-colors">{p.title}</p>
                              <p className="text-[10px] text-gray-400 truncate">{p.currentModule || "Starting soon..."}</p>
                            </div>
                            <span className="text-xs font-bold shrink-0" style={{ color: p.color }}>{p.progress}%</span>
                          </div>
            ...
            <motion.div initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25 }}>
              <Card className="border border-gray-100 shadow-sm">
                ...
                <CardContent>
                  ...
                  <Button variant="outline" className="w-full mt-4 text-xs font-semibold hover:text-[var(--fundi-orange)] hover:border-[var(--fundi-orange)]">
                  ...
                  <div className="border-l-2 border-gray-100 ml-1.5 space-y-5">
                    {dashboardData.upcomingLessons.map((event) => (
                      <div key={`tl-${event.id}`} className="relative pl-6">
                        ...
                        />
                        <p className="text-xs font-bold mb-0.5" style={{ color: event.color }}>{event.fullDate}</p>
                        <p className="text-sm font-bold text-[var(--fundi-black)] leading-snug">{event.microcredential}</p>
