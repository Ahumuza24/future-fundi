...
import {
    ...
    BookOpen, TrendingUp, CheckCheck, XCircle, Loader2,
    type LucideIcon,
} from "lucide-react";
...
/* ─── Color map ───────────────────────────────────────────────────────────── */
const STAT_COLOR_CLASSES: Record<string, { border: string; text: string }> = {
    "fundi-cyan": { border: "border-l-fundi-cyan", text: "text-fundi-cyan" },
    "fundi-purple": { border: "border-l-fundi-purple", text: "text-fundi-purple" },
    "fundi-orange": { border: "border-l-fundi-orange", text: "text-fundi-orange" },
    "fundi-lime": { border: "border-l-fundi-lime", text: "text-fundi-lime" },
    "fundi-red": { border: "border-l-fundi-red", text: "text-fundi-red" },
};
...
/* ─── Stat card ───────────────────────────────────────────────────────────── */
function StatCard({ label, value, sub, colorKey, icon: Icon }:
    { label: string; value: number | string; sub?: string; colorKey: string; icon: LucideIcon }) {
    const colors = STAT_COLOR_CLASSES[colorKey] ?? STAT_COLOR_CLASSES["fundi-cyan"];
    return (
        <Card className={cn("border-l-4", colors.border)}>
            <CardContent className="p-5 flex items-center justify-between">
                ...
                <Icon className={cn("h-10 w-10 opacity-15", colors.text)} />
            </CardContent>
        </Card>
    );
}
...
export default function AdminMonitor() {
    ...
    const tabs: { id: Tab; label: string; icon: LucideIcon; colorKey: string }[] = [
        { id: "sessions", label: "Teacher Sessions", icon: CalendarDays, colorKey: "fundi-cyan" },
        { id: "tasks", label: "Teacher Tasks", icon: ListTodo, colorKey: "fundi-purple" },
        { id: "attendance", label: "Student Attendance", icon: Users, colorKey: "fundi-lime" },
    ];
            ...
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                {loadingSummary ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-3">
                        <StatCard label="Sessions Today" value={sessionsSummary?.today ?? 0} colorKey="fundi-cyan" icon={CalendarDays} />
                        <StatCard label="Sessions / Month" value={sessionsSummary?.this_month ?? 0} colorKey="fundi-cyan" sub="this month" icon={BarChart3} />
                        <StatCard label="Attendance Pending" value={sessionsSummary?.attendance_pending ?? 0} colorKey="fundi-orange" icon={AlertCircle} />
                        <StatCard label="Tasks Open" value={(tasksSummary?.by_status?.todo ?? 0) + (tasksSummary?.by_status?.in_progress ?? 0)} colorKey="fundi-purple" icon={ListTodo} />
                        <StatCard label="Overdue Tasks" value={tasksSummary?.overdue ?? 0} colorKey="fundi-red" icon={XCircle} />
                        <StatCard label="Attendance Rate" value={`${attSummary?.overall_attendance_rate ?? 0}%`} colorKey="fundi-lime" icon={TrendingUp} />
                    </div>
                )}
                    ...
                    {tabs.map(t => (
                        <button key={t.id} onClick={() => setTab(t.id)}
                            className={cn(
                                ...
                                tab === t.id
                                    ? cn(STAT_COLOR_CLASSES[t.colorKey].text, "border-current")
                            ...
                            <motion.div key="attendance" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                                {attSummary && (
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                                        <StatCard label="Present" value={attSummary.by_status?.present ?? 0} colorKey="fundi-lime" icon={CheckCheck} />
                                        <StatCard label="Absent" value={attSummary.by_status?.absent ?? 0} colorKey="fundi-red" icon={XCircle} />
                                        <StatCard label="Late" value={attSummary.by_status?.late ?? 0} colorKey="fundi-orange" icon={Clock} />
                                        <StatCard label="This Month" value={attSummary.this_month ?? 0} colorKey="fundi-cyan" icon={CalendarDays} />
                                    </div>
                                )}
