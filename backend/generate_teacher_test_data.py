...
from apps.core.models import School, Module, Learner, Session, Attendance, Artifact
...
def create_test_data():
    ...
    school, created = School.objects.get_or_create(
        code="TEST001",
        defaults={'name': "Test Future Fundi School"}
    )
    if created:
    ...
    if created:
        ...
        teacher.save()
        print(f"✅ Created teacher: {teacher.get_full_name()} (username: teacher_test, password: teacher123)")
    else:
    ...
    modules_data = [
        ...
        {"name": "Electronics Workshop", "description": "Circuit building and electronics"},
    ]
    ...
    sessions_data = [
        # Today's sessions
        {"module": modules[0], "date": today, "start": time(9, 0), "status": "scheduled"},
        {"module": modules[1], "date": today, "start": time(11, 0), "status": "scheduled"},
        {"module": modules[2], "date": today, "start": time(14, 0), "status": "completed"},
        
        # Tomorrow's sessions
        {"module": modules[0], "date": today + timedelta(days=1), "start": time(9, 0), "status": "scheduled"},
        {"module": modules[3], "date": today + timedelta(days=1), "start": time(13, 0), "status": "scheduled"},
        
        # Next week
        {"module": modules[1], "date": today + timedelta(days=7), "start": time(10, 0), "status": "scheduled"},
    ]
    ...
    for sess_data in sessions_data:
        session, created = Session.objects.get_or_create(
            ...
            defaults={
                ...
                'end_time': time(sess_data["start"].hour + 2, 0) if sess_data["start"] else None,
            }
        )
    ...
    for i, learner in enumerate(learners[:3]):
        artifact, created = Artifact.objects.get_or_create(
            ...
            defaults={
                ...
                'media_refs': [{"type": "photo", "url": f"https://example.com/photo{i}.jpg"}]
            }
        )
        if created:
    ...
    print(f"   - Username: teacher_test")
    print(f"   - Password: teacher123")
    print(f"\n   Parent: {parent.get_full_name()}")
    print(f"   - Username: parent_test")
    print(f"   - Password: parent123")
    print(f"\n   Learners: {len(learners)}")
    for learner in learners:
        print(f"   - {learner.full_name} (username: {learner.first_name.lower()}_test, password: learner123)")
    print(f"\n   Modules: {len(modules)}")
