    ...
    def handle(self, *args, **options):
        ...
        for course in courses:
            ...
            if not levels.exists():
                self.stdout.write(self.style.WARNING(f'  ⚠ No levels found for {course.name}'))
                continue
            ...
            if not modules.exists():
                self.stdout.write(self.style.WARNING(f'  ⚠ No modules found for {course.name}'))
                continue
            
            self.stdout.write(f'  Found {levels.count()} levels and {modules.count()} modules')
            ...
            for idx, level in enumerate(levels):
                ...
                if existing_count > 0:
                    self.stdout.write(f'  ✓ Level {level.level_number}: Already has {existing_count} modules')
                    continue
                
                # Calculate which modules to assign to this level
                start_idx = idx * modules_per_level
                end_idx = start_idx + modules_per_level if idx < levels.count() - 1 else len(modules_list)
        ...
        for course in courses:
            ...
            self.stdout.write(f'{course.name}: {total_modules} modules across {levels.count()} levels')
