...
from apps.core.models import Session, Attendance, Learner, Artifact, Module
...
class TeacherSessionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        ).select_related('tenant', 'teacher', 'module').prefetch_related(
            Prefetch('attendance_records', queryset=Attendance.objects.select_related('learner')),
            'learners'
    ...
    def today(self, request):
        ...
        return Response({
            ...
            'pending': sessions.filter(status__in=['scheduled', 'in_progress']).count(),
        })
    ...
    def dashboard(self, request):
        ...
        today_sessions = Session.objects.filter(
            teacher=teacher,
            date=today
        ).select_related('module').prefetch_related('attendance_records')
        ...
        sessions_needing_artifacts = Session.objects.filter(
            teacher=teacher,
            status='completed',
            date__gte=date.today().replace(day=1)  # This month
        ).annotate(
            artifact_count=Count('learners__artifacts', filter=Q(
                learners__artifacts__submitted_at__date=today
            ))
        ).filter(artifact_count=0).count()
...
class QuickArtifactViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        ).select_related('learner', 'tenant').order_by('-submitted_at')
    ...
    def pending(self, request):
        ...
        sessions = Session.objects.filter(
            ...
            date=today,
            status='completed'
        ).select_related('module').prefetch_related('learners')
        ...
        for session in sessions:
            ...
            artifacts_count = Artifact.objects.filter(
                learner__in=session.learners.all(),
